...
\begin{document}
\maketitle
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[borderline west={3pt}{0pt}{shadecolor}, interior hidden, enhanced, boxrule=0pt, frame hidden, sharp corners, breakable]}{\end{tcolorbox}}\fi
...
\hypertarget{glm---maths-and-theory}{%
\section{GLM - Maths and Theory}\label{glm---maths-and-theory}}

\hypertarget{mean}{%
\subsection{Mean}\label{mean}}

Let \(\eta = x_i^T\beta\) equal the linear predictor, and
\(E(Y_i) = \mu_i\). Then:

\(g(\mu_i) = \eta_i\) and \(g\) is the \textbf{Link}

\(h(\eta_i) = \mu_i\) and \(h\) is the \textbf{Inverse Link}

\(g\) must be monotonic and invertable. It enables the means to be
non-linear in the parameters.

\begin{quote}
The link function converts the mean to the linear predictor. The inverse
link converts the linear predictor to the mean.
\end{quote}
...
\tightlist
\item
  \(\phi\) is a scalar called the scale or dispersion parameter and
  depends on the distribution (it may be known or require estimating and
  can be used when over dispersion is present)
\item
  \(w_i\) are the weights
\item
...
\hypertarget{pdfpmf}{%
\subsection{PDF/PMF}\label{pdfpmf}}
...
(\(\theta\) is used as in terms of \(\mu\) the pdf and moments would
become less elegant)

\hypertarget{exponential-family}{%
\subsection{Exponential family}\label{exponential-family}}

There are a number of distributions that belong to the exponential
family, and it has a useful, tractable properties. These included
Normal, Poisson, Binomial, Gamma and more. Their properties allow us to
calculate the E and Var as before.

\hypertarget{common-exponential-forms}{%
\subsection{Common exponential forms}\label{common-exponential-forms}}

Note the binomial version here is where \(Y\) is the proportion
\([0,1]\)

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.2159}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.0909}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.0568}}
...
\midrule()
\endhead
Normal \(Y\sim N(\theta, \phi)\) & \(\phi\) & 1 & \(\theta\) & 1 &
\(\theta^2 /2\) & \(-(y^2 / \phi +log(2\pi\phi))/2\) \\
Poisson \(Y\sim Po(e^{\theta})\) & 1 & 1 & \(e^{\theta}\) &
\(e^{\theta}\) & \(e^{\theta}\) & \(-log(y!)\) \\
Binomial \(nY\sim Bin(n, e^{\theta}/(e^{\theta} +1)\) & 1 & n &
\(e^{\theta}/(e^{\theta} +1)\) & \(\mu(1-\mu)\) & \(log(e^{\theta} +1)\)
& \(log{n \choose ny}\) \\
\bottomrule()
\end{longtable}
